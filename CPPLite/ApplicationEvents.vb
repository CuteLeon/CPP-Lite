    ...
    Partial Friend Class MyApplication
        ...
        Private Sub MyApplication_Startup(sender As Object, e As StartupEventArgs) Handles Me.Startup
            ...
            If CurrentProcess <> 0 Then
                Debug.Print(String.Format("当前进程虚拟句柄：" & CurrentProcess))
            Else
                Debug.Print(String.Format("获取当前进程虚拟句柄失败,错误代码：" & GetLastError()))
                Exit Sub
            End If

            Dim htRet As Integer = OpenProcessToken(CurrentProcess, TOKEN_ADJUST_PRIVILEGES Or TOKEN_QUERY, hToken)
            If hToken <> 0 Then
                Debug.Print(String.Format("打开进程令牌成功，句柄：" & hToken))
            Else
                Debug.Print(String.Format("打开进程令牌失败,错误代码：" & GetLastError()))
                Exit Sub
            End If
            ...
            If LookRet <> 0 Then
                Debug.Print(String.Format("获取系统特权LUID成功！" & sedebugnameValue.LowPart & " - " & sedebugnameValue.HighPart))
            Else
                Debug.Print(String.Format("获取系统特权LUID失败,错误代码：" & GetLastError()))
                Exit Sub
            End If
            ...
            If ATPErr = 0 Then
                Debug.Print(String.Format("已提高进程权限"))
            Else
                Debug.Print(String.Format("提高进程权限失败,错误代码：" & ATPErr))
                Exit Sub
            End If


            Dim instance As SecurityPermissionAttribute = New SecurityPermissionAttribute(SecurityAction.Assert)
            instance.Assertion = True
            instance.UnmanagedCode = True
            instance.Unrestricted = True
            instance.Flags = SecurityPermissionFlag.AllFlags
            Debug.Print(instance.UnmanagedCode)
